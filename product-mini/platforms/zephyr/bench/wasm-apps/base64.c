...
#define base64_ITERATIONS 1000 // 10000
#define base64_LENGTH 10 // 1000
    ...
    const size_t bin_len, const int variant)
{
    ...
    if(b64_pos > b64_len){
        printf("[base64] assert failed: b64_pos <= b64_len\n");
    }
...
int bench_base64()
{
    ...
    printf("[base64] running with len = %zu for %d iterations\n", len, base64_ITERATIONS);
    bench_start();
    int i;
    for (i = 0; i < base64_ITERATIONS; i++)
    {
        BLACK_BOX(bin);
